...
/** @brief SrvMgr registered name for audio server. */
inline constexpr const char* AUDIO_SRV_NAME = "audio.service";

/**
 * @brief Transaction codes for SrvMgr interface.
 */
enum SrvMgrCode : uint32_t {
    SRV_MGR_GET_SRV = 1,
    SRV_MGR_ADD_SRV = 2,
};
...
/**
 * @brief Transaction codes for LED callback interface.
 */
enum LedSrvCallbackCode : uint32_t {
    LED_SRV_CB_ON_EVENT = 1,
};

/**
 * @brief Transaction codes for audio callback interface.
 */
enum AudioSrvCallbackCode : uint32_t {
    AUDIO_SRV_CB_ON_EVENT = 1,
};
